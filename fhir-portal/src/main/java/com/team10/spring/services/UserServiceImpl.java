...
import com.team10.spring.mail.MailSender;
...
import com.team10.spring.dto.UserDetailsImpl;
...
public class UserServiceImpl implements UserService, UserDetailsService {
    ...
    public void verify(String verificationCode) {
        ...
        MyUtil.validate(user.getRoles().contains(User.Role.UNVERIFIED), "alreadyVerified");
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public void updatePassword(long userId, UserEditPasswordForm userEditPasswordForm) {

        User loggedIn = MyUtil.getSessionUser();
        MyUtil.validate(loggedIn.isAdmin() || loggedIn.getId() == userId, "noPermissions");
        User user = userRepository.findOne(userId);
        user.setPassword(passwordEncoder.encode(userEditPasswordForm.getPassword_new()));
        userRepository.save(user);
    }

    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    public void updateEmail(long userId, UserEditEmailForm userEditEmailForm) {

        User loggedIn = MyUtil.getSessionUser();
        MyUtil.validate(loggedIn.isAdmin() || loggedIn.getId() == userId, "noPermissions");
        User user = userRepository.findOne(userId);
        user.setEmail(userEditEmailForm.getEmail_new());
        userRepository.save(user);
    }
